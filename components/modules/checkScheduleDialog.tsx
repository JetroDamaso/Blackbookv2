...
const CheckScheduleDialog = (props: {
    ...
    const hasConflict = useMemo(() => {
        ...
        const selectedDatesList = dateRange ? selectedDates : selectedDay ? [selectedDay] : [];
        ...
        return selectedDatesList.some(date => {
            ...
            return bookings.some(booking => {
                ...
                const bookingPavilionId = (booking as any).pavilionId;
                ...
                const bookingDateTime = new Date(bookingDate as any);
                ...
                const dayBeforeOnly = new Date(dayBefore.getFullYear(), dayBefore.getMonth(), dayBefore.getDate());
                const dayAfterOnly = new Date(dayAfter.getFullYear(), dayAfter.getMonth(), dayAfter.getDate());

                return (bookingDateOnly.getTime() === dayBeforeOnly.getTime()) ||
                    (bookingDateOnly.getTime() === dayAfterOnly.getTime()) ||
                    (bookingDateOnly.getTime() === selectedDateOnly.getTime());
            });
        });
    }, [selectedPavilionId, selectedDay, dateRange, selectedDates, bookings]);
    ...
    const handleContinue = () => {
        ...
        if (dateRange) {
            ...
            params.set("startDate", `${dateRange.start.year}-${String(dateRange.start.month + 1).padStart(2, '0')}-${String(dateRange.start.day).padStart(2, '0')}`);
            params.set("endDate", `${dateRange.end.year}-${String(dateRange.end.month + 1).padStart(2, '0')}-${String(dateRange.end.day).padStart(2, '0')}`);
        } else if (selectedDay) {
            // For single date
            params.set("startDate", `${selectedDay.year}-${String(selectedDay.month + 1).padStart(2, '0')}-${String(selectedDay.day).padStart(2, '0')}`);
        }
                ...
                <DialogTrigger asChild>
                    <Button className="max-[479px]:aspect-square max-[479px]:p-0! shadow-sm">
                        <Plus
                            ...
                            aria-hidden="true"
                        />
                        <span className="max-[479px]:sr-only flex gap-2 items-center">Add Booking
                            {/* if date is present, show. Else, hide */}
                            {(selectedDay || dateRange) && (
                                <p className="text-xs bg-white rounded-md px-2 py-1 text-black">
                                    {dateRange
                                        ? `${new Date(dateRange.start.year, dateRange.start.month, dateRange.start.day).toLocaleDateString(undefined, {
                                            month: "short",
                                            day: "numeric",
                                        })} - ${new Date(dateRange.end.year, dateRange.end.month, dateRange.end.day).toLocaleDateString(undefined, {
                                            month: "short",
                                            day: "numeric",
                                        })}`
                                        : selectedDay
                                            ? new Date(selectedDay.year, selectedDay.month, selectedDay.day).toLocaleDateString(undefined, {
                                                month: "short",
                                                day: "numeric",
                                            })
                                            : "No day selected"}
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="*:not-first:mt-1">
                            ...
                            <Select value={selectedPavilionId} onValueChange={setSelectedPavilionId}>
                                ...
                                <SelectContent>
                                    {pavilions.map((pav) => (
                                        <SelectItem key={pav.id} value={String(pav.id)}>{pav.name}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </div>
